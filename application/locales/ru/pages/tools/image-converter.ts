...
export const imageConverter: ImageConverterTranslations = {
  ...
  howToUse: {
    ...
    steps: [
      "Перетащите файлы изображений или нажмите для выбора файлов, которые хотите конвертировать",
      "Выберите выходной формат (JPEG, PNG, WebP, BMP)",
      "При необходимости настройте качество и размер",
      "Нажмите кнопку 'Конвертировать' для начала конвертации",
      "Скачайте по отдельности или все вместе после завершения конвертации",
    ],
  },
  ...
  features: {
    ...
    items: [
      "Пакетная конвертация нескольких изображений (до 20 файлов)",
      "Поддержка основных форматов изображений (JPEG, PNG, WebP, BMP)",
      "Настройка качества и оптимизация размера файла",
      "Изменение соотношения сторон и настройка пользовательского размера",
      "Функция предварительного просмотра сравнения до/после",
      "Простая работа перетаскиванием",
    ],
  },
