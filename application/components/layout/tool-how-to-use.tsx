          ...
          <div className="space-y-3 sm:space-y-4">
            {steps.map((step, index) => {
              ...
              return (
                <div
                  key={index}
                  className="flex items-start gap-3 p-3 sm:p-4 bg-white/70 rounded-lg border border-blue-100"
                >
                  <div className="flex-shrink-0 w-6 h-6 sm:w-7 sm:h-7 bg-blue-500 text-white rounded-full flex items-center justify-center text-xs sm:text-sm font-semibold">
                    {index + 1}
                  </div>
                  <div className="flex-1 min-w-0">
                    {stepTitle && (
                      <h4 className="text-sm sm:text-base font-medium text-gray-800 mb-1 break-words">
                        {stepTitle}
                      </h4>
                    )}
        ...
        <Card className="border-green-200 bg-gradient-to-br from-green-50 to-emerald-50">
          <CardHeader className="pb-4">
            <CardTitle className="flex items-center gap-2 text-green-700 text-lg sm:text-xl">
              ...
              <span className="truncate">{features.title}</span>
            </CardTitle>
          </CardHeader>
          <CardContent className="pt-0">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3">
              {features.items.map((feature, index) => {
                ...
                return (
                  <div
                    key={index}
                    className="flex items-start gap-2 sm:gap-3 p-2 sm:p-3 bg-white/70 rounded-lg border border-green-100"
                  >
                    {FeatureIcon ? (
                      <FeatureIcon className="w-3 h-3 sm:w-4 sm:h-4 text-green-500 flex-shrink-0 mt-0.5" />
                    ) : (
                      <div className="w-1.5 h-1.5 sm:w-2 sm:h-2 bg-green-500 rounded-full flex-shrink-0 mt-2" />
                    )}
                    <div className="flex-1 min-w-0">
                      {featureTitle && (
                        <h5 className="text-xs sm:text-sm font-medium text-gray-800 mb-1 break-words">
                          {featureTitle}
                        </h5>
                      )}
