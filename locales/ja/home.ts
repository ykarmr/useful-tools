import { HomeTranslations } from "../types/pages/tools/home";

export const home: HomeTranslations = {
  title: "生産性向上のための包括的無料オンラインツール",
  subtitle:
    "意思決定、タスク管理、生産性向上をサポートする20以上の無料オンラインツールコレクション。",
  keywords: [
    "オンラインツール",
    "無料ツール",
    "生産性ツール",
    "タスク管理",
    "意思決定ツール",
    "ランダム数生成器",
    "ルーレット",
    "TODOリスト",
  ],
  description:
    "包括的な無料オンラインツールコレクションを探索し、賢明な決定を下し、タスクを管理し、生産性を向上させましょう。シンプルなルーレットで決断を下す必要がある場合でも、生活を整理するための強力なTODOリストが必要な場合でも、あなたに合ったツールがあります。",
  tryRoulette: "ルーレットを試す",
  manageTasks: "タスク管理",
  chooseTool: "ツールを選択",
  chooseToolDescription:
    "生産性を高めるために設計された、包括的な無料オンラインツールコレクションからお選びください。",
  rouletteDescription:
    "インタラクティブなルーレットホイールで簡単に決断。オプションを追加して、ホイールに決めてもらいましょう。",
  todoDescription:
    "シンプルながらパワフルなTODOマネージャーで整理整頓。タスクの追跡、優先順位の設定、期限の管理ができます。",
  readyToStart: "生産性を向上させる準備はできていますか？",
  readyToStartDescription:
    "私たちのツールは、よりスマートに、より効率的に作業するためのものです。今すぐ無料でお試しください。",
  startUsingTools: "ツールの使用を開始",
  popularTools: "人気のツール",
  allTools: "すべてのツール",
  featuredTools: "注目のツール",
  utilityTools: "ユーティリティツール",
  getStarted: "始める",
};
